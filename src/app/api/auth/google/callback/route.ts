...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!tokenData.id_token) {
      return NextResponse.redirect(new URL('/login?error=google_id_token_missing', request.url));
    }
    ...
    if (!user) {
      ...
      if (!chosenMobile) {
        return NextResponse.redirect(new URL('/login?error=google_mobile_generation_failed', request.url));
      }
    ...
    const target = user.role === UserRole.CUSTOMER ? isSafeNextPath(parsedState.nextPath) : '/login';
